...
def fastline_033_landing(context: AssetExecutionContext, box: BoxResource):
    """Download infra fastline 033 files from S3 and ingest into public_workspace.raw_noisemap."""
    dept = '033'
    path= f"noisemap/cbs_infra_fastlines/dept={dept}/campaign=2022/"
    return(ingest_from_s3_landing(context,path=path, type="fastline",dept=dept, box=box, folder_id=BOX_FOLDER_033_ID))
...
def fastline_044_landing(context: AssetExecutionContext, box: BoxResource):
    """Download infra fastline 044 files from S3 and ingest into public_workspace.raw_noisemap."""
    dept = '044'
    path= f"noisemap/cbs_infra_fastlines/dept={dept}/campaign=2022/"
    return(ingest_from_s3_landing(context,path=path, type="fastline",dept=dept, box=box, folder_id=BOX_FOLDER_044_ID))