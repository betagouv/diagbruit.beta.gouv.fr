...
def agglo_landing(context: AssetExecutionContext):
    ...
    for entry in mapping:
        shp_path = local_dir / entry["file"]
        if not shp_path.exists():
            context.log.warning(f"File not found, skipping: {entry['file']}")
            skipped += 1
            continue

        context.log.info(f"Ingesting {entry['file']} → raw_noisemap ({entry.get('if_exists', 'append')})")
        success = ingest_shapefile(
            str(shp_path),
            "raw_noisemap",
            _db_url(),
            schema="public_workspace",
            if_exists=entry.get("if_exists", "append"),
            fixed_columns=entry.get("add_columns"),
            column_renames=entry.get("rename_columns"),
